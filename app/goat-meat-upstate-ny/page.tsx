...
const faqs = [
  ['Where can I buy goat meat in Upstate New York?', 'Willow Anchor Farm raises and sells pasture-raised goat meat from the Town of Florida in Montgomery County, New York. Farm pickup is available by arrangement. Email the farm for current availability and pricing.'],
  ...
  ['Is delivery available?', 'Delivery may be available for larger orders across the farm’s Upstate New York service area. Include your town, county, and desired quantity in your inquiry.'],
]
...
const farmSchema = {
  ...
  areaServed: ['Montgomery County', 'Albany County', 'Saratoga County', 'Fulton County', 'Schoharie County'],
...
export default function GoatMeatPage() {
  return (
    <div className="min-h-screen overflow-x-hidden bg-white">
      ...
      <main>
        <section id="top" className="relative flex min-h-[720px] items-end overflow-hidden bg-black pt-20 md:min-h-[780px]">
          <Header />
          <Image src="/meat-goats.jpg" alt="Adirondack Meat Goats grazing at Willow Anchor Farm in Montgomery County, New York" fill priority sizes="100vw" className="object-cover object-center" />
          ...
          <div className="container relative z-10 mx-auto px-5 pb-16 pt-36 md:px-8 md:pb-24">
            <div className="max-w-3xl">
              <p className="mb-4 text-sm font-semibold uppercase tracking-[0.2em] text-white/75">Town of Florida · Montgomery County, NY</p>
              <h1 className="text-5xl font-semibold leading-[1.05] tracking-tight text-white sm:text-6xl lg:text-7xl">Pasture-raised goat meat in Upstate New York.</h1>
              <p className="mt-6 max-w-2xl text-lg leading-relaxed text-white/85 md:text-xl">Willow Anchor Farm raises Savanna and Boer heritage goats on pasture. Ask about individual cuts or ordering an animal with a cut sheet.</p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <a href={goatInquiryHref} className="inline-flex items-center justify-center rounded-full bg-white px-6 py-3.5 font-semibold text-[#3f5b38] hover:bg-white/90">Check availability and pricing <ArrowRight className="ml-2 h-4 w-4" /></a>
                <a href="#how-to-buy" className="inline-flex items-center justify-center rounded-full border border-white/70 px-6 py-3.5 font-semibold text-white hover:bg-white/10">How ordering works</a>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white py-20 md:py-28">
          <div className="container mx-auto grid gap-12 px-5 md:px-8 lg:grid-cols-[1fr_.9fr] lg:items-center">
            <div>
              ...
              <h2 className="text-4xl font-semibold tracking-tight text-stone-900 md:text-5xl">Raised with pasture at the center.</h2>
            ...
            <div className="rounded-3xl bg-[#f6f5f0] p-7 md:p-10">
        ...
        <section id="how-to-buy" className="bg-[#f6f5f0] py-20 md:py-28">
          <div className="container mx-auto px-5 md:px-8">
            ...
            <div className="mx-auto mt-12 grid max-w-5xl gap-5 md:grid-cols-3">
              ...
              ].map(([number, title, copy]) => <div key={number} className="rounded-2xl bg-white p-7 shadow-sm"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#4A6741] font-semibold text-white">{number}</span><h3 className="mt-5 text-xl font-semibold text-stone-900">{title}</h3><p className="mt-3 leading-relaxed text-stone-600">{copy}</p></div>)}
            </div>
            <div className="mt-9 text-center"><a href={goatInquiryHref} className="inline-flex items-center rounded-full bg-[#4A6741] px-6 py-3 font-semibold text-white hover:bg-[#3f5b38]">Ask about goat meat <ArrowRight className="ml-2 h-4 w-4" /></a></div>
          </div>
        </section>

        <section className="bg-[#4A6741] py-20 text-white md:py-24">
          <div className="container mx-auto grid gap-10 px-5 md:px-8 lg:grid-cols-[1fr_.9fr] lg:items-center">
            <div>
              ...
              <h2 className="text-4xl font-semibold tracking-tight md:text-5xl">Goat meat from Montgomery County.</h2>
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-white/80">Willow Anchor Farm is in the Town of Florida. We welcome inquiries from Montgomery, Albany, Saratoga, Fulton, and Schoharie counties, including communities such as Latham, Bethlehem, and Clifton Park.</p>
            </div>
            <div className="rounded-3xl bg-white/10 p-7 backdrop-blur-sm md:p-9">
              <MapPin className="h-8 w-8 text-white/80" />
              <h3 className="mt-5 text-2xl font-semibold">Pickup and delivery</h3>
              <p className="mt-3 leading-relaxed text-white/75">Farm pickup is available by arrangement. Because the address is private, pickup details are shared after an order is confirmed. Delivery may be available for larger orders.</p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-[#e8ede4] py-16 md:py-20">
          <div className="container mx-auto px-5 text-center md:px-8">
            ...
            <a href={goatInquiryHref} className="mt-7 inline-flex items-center rounded-full bg-[#4A6741] px-6 py-3 font-semibold text-white hover:bg-[#3f5b38]">Email Willow Anchor Farm <ArrowRight className="ml-2 h-4 w-4" /></a>
          </div>
        </section>
      </main>
      ...
      <footer className="bg-stone-950 py-12 text-white">
        <div className="container mx-auto flex flex-col gap-8 px-5 md:flex-row md:items-center md:justify-between md:px-8">
          ...
          <div className="flex flex-col gap-3 text-sm sm:flex-row sm:gap-6">
            ...
            <a href={`mailto:${contactEmail}`} className="inline-flex items-center text-white/75 hover:text-white"><Mail className="mr-2 h-4 w-4" />{contactEmail}</a>
